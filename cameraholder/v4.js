...
function cs(r, p) {
  ...
  if ( p ) {
      for ( var i = 0 ; i < 4 ; i++ ) {
     var c = cube([r*p,r*p,2]).subtract(cylinder({fn:100,r: r*p,h:2}));
...
function main() {
  ...
  for ( var i = 0 ; i < 20 ; i += interp(0.05,0.25,i/20) ) {
    var t = cs(interp(62/2, 45/2, interp(Math.sqrt(i/20), Math.pow(i/20,0.1), i/20)), Math.sqrt((i+0.25)/20.25)).translate([0,0,i*0.98]);
    t = t.rotateY(i*2)
    s.push(t);
  }
  ...
  s = s.union(s.translate([0,0,-2]))
  s = s.union(s.translate([0,0,-3]))
