...
function createArm() {
  ...
  for ( var i = 0 ; i < 1 ; i+= 0.01 ) {
     ...
     if ( i > 0.15 ) {
         ...
         s = s.translate([0,-1,45*i]).rotateX(24*i);
     a.push(s);
  }
...
function main() {
  ...
  b = b.subtract(cylinder({r:3.1/2,h:100, fn: FN}).rotateY(90).translate([-50,4,10.6]))
  b = b.union(cylinder({r:4.8/2,h:11, fn: FN}).rotateY(90).translate([-5.5,-4,10.6]))
  b = b.subtract(cylinder({r:3.1/2,h:100, fn: FN}).rotateY(90).translate([-50,-4,10.6]))

  b = b.union(cylinder({r:4.8/2,h:11, fn: FN}).rotateY(90).translate([-5.5,-12.5,48]))
  b = b.subtract(cylinder({r:3.1/2,h:100, fn: FN}).rotateY(90).translate([-5.5,-12.5,48]))
