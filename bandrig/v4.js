...
function f(t) {
  var tx = t + Math.sin(t*Math.PI)*1;
  t = t * Math.sqrt(Math.sqrt(Math.sqrt(t))) + t * (1-t);
  return cube({size:[1+t*t*t*1.62, 1+t*t*t*t*0.8, 0.5+t*t*t/2], round: true, center: true}).translate([3*tx,0,4*0.7*t*t*t*t*t*t*t*t*t*t*t*t*t*t*t*t*t*t]);
}
...
function main() {
  var s = merge(f, 50).translate([-0.5, 0,0]);
