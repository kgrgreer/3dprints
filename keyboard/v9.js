...
function createFinger(m) {
    ...
    var f = Object.assign({
        ...
        toSolid: memoize(function() {
          ...
          base = trimZ(base);
          if ( BASE ) {
            var sh = hull(baseShadow(base)).contract(10);
          ...
          var b = cube({size:[bounds[1].x-bounds[0].x,bounds[1].y-bounds[0].y, 25]});
          b = b.translate([bounds[0].x,bounds[0].y,0]);
          b = b.subtract(this.neg);
          b = b.subtract(sphere({r:this.r+5}).translate([0,0,this.height]).translate(this.translate));
          base = base.union(b);
          } else {
              /*
